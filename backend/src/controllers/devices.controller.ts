...
/** How many installs the test actually went to. */
...
export class DevicesController {
  ...
  /**
   * Sends a push straight away, so the owner can hear whether it arrives
   * without waiting for something to fall due.
   */
  async test(_request: Request, response: Response<TestResult>, next: NextFunction): Promise<void> {
    try {
      const delivered = await this.devicesService.sendTest(currentUserId(response));
      SuccessHandlerUtil.handleList(response, next, { delivered });
    } catch (error) {
      next(error);
    }
  }
